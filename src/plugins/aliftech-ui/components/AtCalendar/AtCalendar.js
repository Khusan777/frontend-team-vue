    ...
    const daysBeforeCurrentMonthDays = computed(() => {
      ...
      return days.map(day =>
        h(
          'button',
          {
            class: 'transition duration-150 text-gray-300 p-2 w-auto text-center hover:text-gray-400',
                  ...
                  getPickerButtonClasses(
                    props.range
                      ? dateRange.value.some(
                          date => date.getDate() === day.date && date.getMonth() + 1 === currentMonth.value
                        )
                      : day.date === selectedDay.value && selectedMonth.value === currentMonth.value
                  ),
                  checkDayBetweenDateRange(day) ? 'bg-' + uiConfig.primaryBackgroundColor + '-100' : '',
                ],
                onClick: () => {
                  props.range ? setDateRange(day, currentMonth.value - 1, selectedYear.value) : setCurrentDateDay(day);
                },
              },
              day.date
            );
        ...
        ? h(
            ...
            [
              ...
              this.type === 'date'
                            ...
                            ? h(
                                ...
                                [
                                  h('div', { class: 'grid grid-cols-2 items-center justify-center' }, [
                                    h('div', { class: 'max-h-48 overflow-y-auto border-r-2 border-gray-200 px-2' }, [
                                      shortMonths.map((month, index) =>
                                        h(
                                          'button',
                                          {
                                            ...
                                            onClick: () => (this.range ? null : this.setMonth(index)),
                                          },
                                          capitalize(month)
                                        )
                                      ),
                                    ]),
                                    h('div', { class: 'max-h-48 overflow-y-auto px-2' }, [
                                      this.yearsToSelect.map(year =>
                                        h(
                                          'button',
                                          {
                                            ...
                                            onClick: () => (this.range ? null : this.setYear(year)),
                                          },
                                          year
                                        )
                                      ),
                ...
                : h('span', { class: 'font-semibold p-1 px-3 rounded-md' }, this.headerTitle),
